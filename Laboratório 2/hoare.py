...
ARQUIVOS = {
    ...
    'OUTPUT_1' : 'stats-aleatorio-hoare.txt',
    'OUTPUT_2' : 'stats-mediana-hoare.txt',
}
...
# pega o primeiro elemento como pivô, além disso, agrupa os elementos menores do que o pivô no início do vetor e agrupa os elementos
# maiores do que o pivô no final do vetor.

def hoare_particao(vetor, inicio, fim, swaps):
    global SWAPS

    # Guarda o elemento que está no início do vetor
    elemento_particao = vetor[inicio]

    # Guarda o elemento anterior ao início
    i = (inicio - 1)

    # Guarda o elemento seguinte ao fim
    j = (fim + 1)

    while(True):
        i += 1
        # Elemento próximo do ínicio é menor do que o particionador
        while(vetor[i] < elemento_particao):
            i += 1

        j -= 1
        # Elemento próximo do fim é maior do que o particionador
        while(vetor[j] > elemento_particao):
            j -= 1

        # Se o índice do início for maior ou igual ao índice do fim
        if(i >= j):
            return j
...
# organiza um vetor com base em um número aleatório e com base no particionamento de Hoare.

def hoare_randomico(vetor, inicio, fim, swaps):
    global RECURSAO
    global SWAPS
    if fim > inicio:
        RECURSAO += 1
        ...
        # particao guarda o retorno da função hoare_particao
        particao = hoare_particao(vetor, inicio, fim, SWAPS)
        # Chamadas recursivas
        hoare_randomico(vetor, inicio, particao, SWAPS)
        hoare_randomico(vetor, particao + 1, fim, SWAPS)
...
# organizar um vetor com base em um número escolhido entre três e com base no particionamento de Hoare.

def hoare_mediana(vetor, inicio, fim, swaps):
    global RECURSAO
    global SWAPS
    if fim > inicio:
        RECURSAO += 1
        ...
        particao = hoare_particao(vetor, inicio, fim, SWAPS)
        # Chamadas recursivas
        hoare_mediana(vetor, inicio, particao, SWAPS)
        hoare_mediana(vetor, particao + 1, fim, SWAPS)
...
with open(ARQUIVOS.get('INPUT_1'), 'r') as entrada:
    ...
    for linha in total_linhas:
        ...
        hoare_randomico(vetor, 0, tamanho - 1, SWAPS)
...
with open(ARQUIVOS.get('INPUT_1'), 'r') as entrada:
    ...
    for linha in total_linhas:
        ...
        tempo_inicial = t.time()
        # Chamada da função criada
        hoare_mediana(vetor, 0, tamanho - 1, SWAPS)
        # Cronometra o tempo final
        tempo_final = t.time() - tempo_inicial
